...
OUTLOOK_PROXY_API_BASE = "https://mail.chatai.codes"
OUTLOOK_PROXY_FETCH_TIMEOUT = 6
OUTLOOK_PROXY_IMAP_RETRIES = 2
OUTLOOK_PROXY_SECURITY_LOCK = threading.RLock()
OUTLOOK_PROXY_SECURITY_SESSION = None
...
def proxy_http_session(timeout=OUTLOOK_PROXY_FETCH_TIMEOUT, prefer_requests=False):
    if prefer_requests and requests is not None:
        ...
        session.headers.update({
            ...
            "Origin": OUTLOOK_PROXY_API_BASE.rstrip("/"),
            "Referer": OUTLOOK_PROXY_API_BASE.rstrip("/") + "/",
            "Accept": "*/*",
        })
        return session
    ...
    if CurlSession is None:
        return None
    session = CurlSession(impersonate="chrome142")
    session.headers.update({
        "User-Agent": (
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
            "AppleWebKit/537.36 (KHTML, like Gecko) "
...
def post_json(url, payload, headers=None, timeout=REQUEST_TIMEOUT_SECONDS, prefer_requests=False):
    request_headers = {
    ...
    curl_session = proxy_http_session(timeout, prefer_requests=prefer_requests)
    if curl_session is not None:
        try:
            response = curl_session.post(url, headers=request_headers, data=json.dumps(payload), timeout=timeout)
            return int(response.status_code), parse_maybe_json(response.text)
        except Exception:
            pass

    request = Request(url, data=json.dumps(payload).encode("utf-8"), headers=request_headers)
    try:
        with urlopen(request, timeout=timeout) as response:
            text = response.read().decode("utf-8", errors="ignore")
            return response.getcode(), parse_maybe_json(text)
    except HTTPError as exc:
        body = exc.read().decode("utf-8", errors="ignore")
        return int(getattr(exc, "code", 0) or 0), parse_maybe_json(body or str(exc))
...
def get_outlook_proxy_security_session(force_refresh=False):
    global OUTLOOK_PROXY_SECURITY_SESSION
    if AESGCM is None:
        raise RuntimeError("缺少 cryptography 依赖，无法生成 mail.chatai.codes 安全签名")

    now_ms = int(time.time() * 1000)
    with OUTLOOK_PROXY_SECURITY_LOCK:
        cached = OUTLOOK_PROXY_SECURITY_SESSION if isinstance(OUTLOOK_PROXY_SECURITY_SESSION, dict) else None
        if (
            not force_refresh
            and cached
            and int(cached.get("expiresAtMs") or 0) - now_ms > 60_000
        ):
            return cached

        url = f"{OUTLOOK_PROXY_API_BASE.rstrip('/')}/api/security-session"
        status, data = post_json(url, {}, timeout=10, prefer_requests=True)
        if status < 200 or status >= 300 or not isinstance(data, dict) or not data.get("success"):
            raise RuntimeError(f"安全会话初始化失败 HTTP {status}: {describe_payload(data)}")

        expires_at_ms = 0
        raw_expires = str(data.get("expiresAt") or "").strip()
        if raw_expires:
            try:
                expires_at_ms = int(datetime.fromisoformat(raw_expires.replace("Z", "+00:00")).timestamp() * 1000)
            except Exception:
                expires_at_ms = now_ms + 10 * 60_000

        OUTLOOK_PROXY_SECURITY_SESSION = {
            "sessionId": data.get("sessionId") or "",
            "sessionToken": data.get("sessionToken") or "",
            "sessionKey": data.get("sessionKey") or "",
            "expiresAtMs": expires_at_ms or now_ms + 10 * 60_000,
        }
        return OUTLOOK_PROXY_SECURITY_SESSION


def create_outlook_proxy_secure_envelope(payload):
    session = get_outlook_proxy_security_session()
    key = base64url_decode(session["sessionKey"])
    iv = os.urandom(12)
    nonce = base64url_encode(os.urandom(16))
    timestamp = int(time.time() * 1000)
    plaintext = json.dumps(payload, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    ciphertext = AESGCM(key).encrypt(iv, plaintext, None)
    iv_text = base64url_encode(iv)
    ciphertext_text = base64url_encode(ciphertext)
    signed_text = f"{session['sessionId']}.{nonce}.{timestamp}.{iv_text}.{ciphertext_text}"
    signature = hmac.new(key, signed_text.encode("utf-8"), hashlib.sha256).digest()
    return {
        "secure": True,
...
def post_outlook_proxy_json(path, payload, timeout=OUTLOOK_PROXY_FETCH_TIMEOUT):
    url = f"{OUTLOOK_PROXY_API_BASE.rstrip('/')}{path}"
    envelope = create_outlook_proxy_secure_envelope(payload)
    status, data = post_json(url, envelope, timeout=timeout, prefer_requests=True)
    detail = describe_payload(data)
    if status in (401, 403) and ("安全签名" in detail or "security" in detail.lower() or "signature" in detail.lower()):
        get_outlook_proxy_security_session(force_refresh=True)
        envelope = create_outlook_proxy_secure_envelope(payload)
        status, data = post_json(url, envelope, timeout=timeout, prefer_requests=True)
    return status, data


def is_transient_proxy_error(text):
...
def is_graph_filter_error(text):
    text = str(text or "").lower()
    return "inefficientfilter" in text or "restriction or sort order is too complex" in text
...
def describe_payload(payload):
    ...
    if isinstance(payload, dict):
        for key in ("message", "msg", "error", "status", "code"):
...
def message_timestamp_ms(item):
    if not isinstance(item, dict):
        return 0
    for key in ("receivedTimestamp", "timestamp", "time"):
        value = item.get(key)
        ...
        try:
            ...
            pass
    raw = str(item.get("date") or item.get("receivedDateTime") or item.get("createTime") or item.get("receivedAt") or "").strip()
    if not raw:
        return 0
    try:
...
def normalize_proxy_message(item, protocol=""):
    if not isinstance(item, dict):
        return {}
    ...
    sender = first("sendEmail", "fromEmail", "from.emailAddress.address", "from")
    sender_name = first("sendName", "fromName", "from.emailAddress.name")
    body = first("text", "bodyText", "bodyPreview", "content", "html", "body.content", "bodyHtml", "body")
    if isinstance(sender, dict):
        sender_name = sender.get("name") or sender_name
        sender = sender.get("address") or sender.get("email") or sender.get("EmailAddress") or ""
    if isinstance(body, dict):
        body = body.get("content") or body.get("text") or ""
    ts = message_timestamp_ms(item)
    return {
        "id": str(first("id", "messageId", "uid", "internetMessageId") or ""),
        "mailbox": str(item.get("mailbox") or protocol or ""),
        "subject": str(first("subject", "Subject") or ""),
        "from": {
            "emailAddress": {
                "address": str(sender or "").strip(),
                "name": str(sender_name or "").strip(),
            }
        },
        "bodyPreview": re.sub(r"<[^>]+>", " ", str(body or ""))[:500],
        "body": {"content": str(body or "")},
        "receivedDateTime": datetime.fromtimestamp(ts / 1000, tz=timezone.utc).isoformat().replace("+00:00", "Z") if ts else str(first("date", "receivedDateTime") or ""),
...
def fetch_proxy_messages(email_addr, client_id, refresh_token, top=5):
    request_payload = {
        "email": email_addr,
        "clientId": client_id,
    ...
    def fetch(protocol):
        max_attempts = OUTLOOK_PROXY_IMAP_RETRIES + 1 if protocol == "imap" else 1
        last_error = ""
        for attempt in range(1, max_attempts + 1):
            started = time.time()
            status, data = post_outlook_proxy_json(f"/api/fetch-{protocol}", request_payload, timeout=OUTLOOK_PROXY_FETCH_TIMEOUT)
            elapsed = round(time.time() - started, 3)
            if status < 200 or status >= 300:
                last_error = f"{protocol} HTTP {status}: {describe_payload(data)}"
            elif isinstance(data, dict) and data.get("success") is False:
                last_error = f"{protocol}: {describe_payload(data)}"
            else:
                raw_messages = data.get("emails") if isinstance(data, dict) else []
                messages = [normalize_proxy_message(item, protocol) for item in (raw_messages or []) if isinstance(item, dict)]
                return {
                    "mailbox": protocol,
                    "transport": protocol,
                    "messages": messages,
            ...
            if attempt < max_attempts and is_transient_proxy_error(last_error):
    ...
    try:
        futures = {executor.submit(fetch, protocol): protocol for protocol in ("graph", "imap")}
        pending = set(futures)
        while pending:
            done, pending = wait(pending, timeout=OUTLOOK_PROXY_FETCH_TIMEOUT + 1, return_when=FIRST_COMPLETED)
            if not done:
                for future in list(pending):
                    protocol = futures[future]
                    future.cancel()
                    errors.append(f"{protocol}: 请求超时")
                break
            for future in done:
                protocol = futures[future]
                try:
                    mailbox_results.append(future.result(timeout=0))
                except Exception as exc:
                    detail = str(exc)
                    if protocol == "graph" and is_graph_filter_error(detail):
                        errors.append(f"{protocol}: Graph 过滤不兼容，已由 IMAP 接管")
                    else:
                        errors.append(f"{protocol}: {detail}")

            if any(result.get("messages") for result in mailbox_results):
                for future in list(pending):
                    future.cancel()
                break
    finally:
    ...
    if not mailbox_results and errors:
        raise RuntimeError("mail.chatai.codes 取信失败：" + " | ".join(errors))
    return {
        "transport": "mail.chatai.codes",
        "token_payload": {"next_refresh_token": refresh_token, "token_endpoint": "mail.chatai.codes"},
...
def collect_messages(email_addr, client_id, refresh_token, mailboxes, top):
    try:
        log_info("message collection start transport=mail.chatai.codes")
        result = fetch_proxy_messages(email_addr, client_id, refresh_token, top)
        log_info(
            "message collection success transport=mail.chatai.codes "
            f"messages={len(result.get('messages') or [])} "
            f"errors={len(result.get('errors') or [])}"
        )
        return result
    except Exception as proxy_exc:
        proxy_message = compact_text(str(proxy_exc), 600)
        log_info(f"message collection failed transport=mail.chatai.codes detail={proxy_message}")

    errors = [f"mail.chatai.codes: {proxy_message}"]
    collectors = [
    ...
    for transport_name, collector in collectors:
        try:
            log_info(f"message collection fallback start transport={transport_name}")
            result = collector(email_addr, client_id, refresh_token, mailboxes, top)
            log_info(
                f"message collection fallback success transport={transport_name} "
            ...
            log_info(f"message collection fallback failed transport={transport_name} detail={message}")
